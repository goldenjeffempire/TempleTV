...
**Follow-on gap found in production (2026-07-03):** `admin-videos.routes.ts` `toDto()` computed the admin UI's `sourceAvailable` flag purely from `objectPath`/`sourceCleanupStatus`/`sourceDeletedAt`, never checking `transcodingErrorCode`. Once a video was confirmed `SOURCE_MISSING` (terminal), those cleanup columns were untouched (they track a *different*, intentional post-transcode-cleanup flow), so `sourceAvailable` stayed `true` and the admin UI offered a futile "Retry Transcoding" button that re-hit the identical missing-blob error every click, instead of "Re-upload required". Fix: `transcodingErrorCode === "SOURCE_MISSING"` must short-circuit `sourceAvailable` to `false` ahead of the cleanup-column checks. Any future terminal error code added per the numbered list above must get the same UI-flag treatment, not just backend classification.
