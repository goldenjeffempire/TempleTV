...
Third recreation: versionCode 94 (v1.0.34), build `ebed4f95`. Same 3 files recreated again — the directory disappears between Replit agent sessions because git only tracks committed files and the android/ directory may not be committed when the session ends.

**Critical note:** The `android/` directory inside `expo-pip-android` is NOT auto-generated — it must exist in source control. Verify it's present before ANY EAS submission: `find artifacts/mobile/modules/expo-pip-android/android -name "*.kt" | grep -q .` If the command produces no output, recreate the 3 files listed above BEFORE submitting. This check must be the first step of any mobile build task.
