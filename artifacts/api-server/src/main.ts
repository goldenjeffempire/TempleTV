...
/**
 * Emit a structured "remediation report" log at boot showing the count of known
 * pipeline health issues across the broadcast queue and transcoder. Designed for
 * on-call engineers: a single scan at startup surfaces what needs human attention
 * without requiring a tour of the admin diagnostics pages.
 *
 * Counts:
 *   • hlsStorageMissing  — active queue items where hls_master_url is set but
 *                          the storage blob is absent (transcoder crash window)
 *   • stuckEncoding      — videos stuck in transcodingStatus='encoding' with no
 *                          active transcoding_jobs row (orphaned after crash)
 *   • placeholderDuration— active queue items still carrying the 1800 s placeholder
 *                          (ffprobe never ran or lost its result)
 *   • failedRecoverable  — transcoding_jobs rows with status='failed' whose error
 *                          code is NOT terminal (CORRUPT_SOURCE / SOURCE_MISSING)
 *                          and whose attempt budget is not exhausted
 *   • failedTerminal     — permanently failed jobs (CORRUPT_SOURCE / SOURCE_MISSING)
 *
 * Fire-and-forget — any DB error is caught and logged at WARN, never surfaces to
 * the caller and never prevents HTTP requests from being served.
 */
async function runBootRemediationReport(): Promise<void> {
  // Delay to let the DB pool warm up and the orchestrator boot before scanning.
  await new Promise<void>((r) => setTimeout(r, 15_000));
  try {
    const { db, sql: dbSql } = await import("./infrastructure/db.js");

    type CountRow = { n: string };

    const [hlsStorage, stuckEnc, placeholder, failedRec, failedTerm] = await Promise.all([
      // 1. HLS storage missing: active items where DB says hls_ready but blob absent
      db.execute<CountRow>(dbSql`
      ...
      `),
      // 2. Stuck encoding: videos at 'encoding' with no active job row
      db.execute<CountRow>(dbSql`
        SELECT COUNT(*)::text AS n
        FROM managed_videos mv
        WHERE mv.transcoding_status = 'encoding'
          AND mv.updated_at < now() - interval '2 hours'
          AND NOT EXISTS (
            SELECT 1 FROM transcoding_jobs tj
            WHERE tj.video_id = mv.id AND tj.status IN ('queued', 'processing')
          )
      `),
      // 3. Placeholder duration: active queue items still at 1800 s
      db.execute<CountRow>(dbSql`
        SELECT COUNT(*)::text AS n
        FROM broadcast_queue
        WHERE is_active = true AND duration_secs = 1800
      `),
      // 4. Failed jobs — recoverable (error code not terminal, budget not exhausted)
      db.execute<CountRow>(dbSql`
        SELECT COUNT(*)::text AS n
      ...
      `),
      // 5. Failed jobs — terminal (CORRUPT_SOURCE / SOURCE_MISSING)
      db.execute<CountRow>(dbSql`
        SELECT COUNT(*)::text AS n
        FROM transcoding_jobs tj
        INNER JOIN managed_videos mv ON mv.id = tj.video_id
        WHERE tj.status = 'failed'
          AND mv.transcoding_error_code IN ('CORRUPT_SOURCE', 'SOURCE_MISSING')
      `),
    ]);

    const toN = (res: { rows: CountRow[] }) => parseInt((res.rows[0] as CountRow | undefined)?.n ?? "0", 10);
    const report = {
      hlsStorageMissing:  toN(hlsStorage as { rows: CountRow[] }),
      stuckEncoding:      toN(stuckEnc as { rows: CountRow[] }),
      placeholderDuration: toN(placeholder as { rows: CountRow[] }),
      failedRecoverable:  toN(failedRec as { rows: CountRow[] }),
      failedTerminal:     toN(failedTerm as { rows: CountRow[] }),
    };

    const hasIssues = Object.values(report).some((v) => v > 0);
    if (hasIssues) {
      logger.warn(report, "[remediation-report] boot pipeline health report — issues detected");
    } else {
      logger.info(report, "[remediation-report] boot pipeline health report — all clear");
    }
  } catch (err) {
    logger.warn({ err }, "[remediation-report] boot report scan failed (non-fatal)");
  }
}
