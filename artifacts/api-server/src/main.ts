...
async function main() {
  ...
  if (mode === "worker" || mode === "all") {
    ...
    // Push the alert 30 s after the FIRST admin SSE client connects, so the
    // operator's dashboard is guaranteed to be listening when it arrives.
    // An immediate push at startup is lost if no browser tab has connected
    // yet.  A 5-min wall-clock fallback ensures the alert fires even when
    // no SSE client ever connects (email escalation picks it up at 10 min).
    void (async () => {
      try {
        ...
        if (aeb.listenerCount("admin-event") > 1) {
          ...
          const onNewListener = (eventName: string) => {
            if (eventName !== "admin-event" || scheduled) return;
            scheduled = true;
            pushAlert();
          };
          aeb.on("newListener", onNewListener);
          // Fallback: push at 5 min even if no SSE client ever connects —
          // the unacked-alert sweeper will escalate to email at 10 min.
          const fallback = setTimeout(() => {
            aeb.off("newListener", onNewListener);
            if (!scheduled) { scheduled = true; pushAlert(); }
          }, 5 * 60_000);
          fallback.unref?.();
        }
      } catch { /* non-fatal */ }
