    ...
    async () => {
      ...
      const [
        ...
        db
          .select({ c: count() })
          .from(videos)
          .where(and(eq(videos.videoSource, "local"), eq(videos.transcodingStatus, "ready"))),
        ...
        localVideos === 0
          ...
          : hlsReadyLocalVideos < localVideos
            ? {
                ...
                action: "Wait for transcoding to finish or investigate stuck jobs",
              }
            : {
                key: "local-uploads",
                label: "Local uploads transcoded",
      ...
      const accessChecks: Check[] = [
        adminUsers === 0
          ? {
              ...
              action: "Create at least one admin user via /admin/auth/seed",
            }
          : {
              key: "admin-users",
              label: "Admin / editor accounts exist",
