...
export async function liveIngestRoutes(app: FastifyInstance) {
  ...
  r.get(
    "/live-ingest/endpoints",
    {
      ...
      schema: {
        ...
        response: { 200: z.object({ endpoints: z.array(EndpointSchema) }) },
        security: [{ bearerAuth: [] }],
      },
    },
    async () => {
      ...
      return { endpoints: rows.map(toDto) };
    },
  );
  ...
  r.post(
    ...
    async (req) => {
      ...
      // If this is the first ever row, force-promote it to primary so
      // there's always exactly one default. If the caller asked for
      // primary, demote any current primary in the same transaction.
      const existingCount = await db.select({ id: ingest.id }).from(ingest).limit(1);
      const shouldBePrimary = body.isPrimary === true || existingCount.length === 0;

      if (shouldBePrimary) {
        await db.update(ingest).set({ isPrimary: false, updatedAt: now });
      }
  ...
  r.post(
    ...
    async (req) => {
      const { id } = req.params as { id: string };
      const now = new Date();
      // Demote then promote in two statements — Drizzle / Neon doesn't
      // give us a clean way to do this in a single SQL UPDATE without
      // a CASE expression that is brittle to schema changes.
      await db
        .update(ingest)
  ...
  r.post(
    ...
    async (req) => {
      const body = (req.body ?? {}) as { staleMs?: number };
      const staleMs = body.staleMs ?? 5 * 60_000; // 5 minutes default
      const cutoff = new Date(Date.now() - staleMs);

      // Anything still claiming `healthy` whose last probe is older than
      // the cutoff is, by definition, no longer known-healthy.
      const rows = await db
        .select({
          id: ingest.id,
          lastHealthAt: ingest.lastHealthAt,
          healthStatus: ingest.healthStatus,
        })
        .from(ingest);
      let swept = 0;
      for (const row of rows) {
        if (row.healthStatus === "healthy" && (!row.lastHealthAt || row.lastHealthAt < cutoff)) {
          await db
            .update(ingest)
            .set({
              healthStatus: "unknown",
            ...
            .where(eq(ingest.id, row.id));
          swept++;
        }
      }
      return { ok: true as const, sweptCount: swept };
    },
  );
