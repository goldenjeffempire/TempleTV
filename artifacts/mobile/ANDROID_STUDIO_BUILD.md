...
| `A problem occurred starting process 'command 'node''` / `Cause: error=2, No such file or directory` | Android Studio (launched from the desktop icon) doesn't inherit your shell's `PATH`, so Gradle can't find `node`. **Fix (pick one):** (1) **Easiest:** quit Android Studio, then launch it from a terminal (`studio.sh` on Linux, `open -a "Android Studio"` on macOS) so it inherits your shell PATH. (2) Symlink node into a system path: `sudo ln -s "$(which node)" /usr/local/bin/node`. (3) Add the absolute path to `android/gradle.properties`: `nodeExecutableAndArgs=/absolute/path/to/node` (find with `which node`). After any of these, in Android Studio: **File → Invalidate Caches → Invalidate and Restart**, then rebuild. |
