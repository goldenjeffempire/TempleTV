...
One-shot setup: chains prebuild → SDK detection → `local.properties` write → Node path helper for native Android builds. Run once after a fresh checkout, or any time `local.properties` is missing.
