      ...
      const p = new window.YT!.Player(containerId.current, {
        ...
        events: {
          onReady: (e: { target: YTPlayer }) => {
            ...
            // The IFrame API creates the <iframe> with fixed pixel width/height
            // attributes (default 640×390). Strip those and replace with
            // position:absolute fill so the iframe is always 100% of its
            // container regardless of screen width or orientation.
            try {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── YouTube chrome blockers ──────────────────────────────────────
          Absolutely-positioned DOM divs in the same stacking context as
          the YouTube <iframe> (both live inside the same React/HTML
          document). High z-index causes them to render on top of the
          cross-origin iframe, covering YouTube's own branded UI without
          needing any cross-origin CSS access.

          • Top gradient cap  — fades from opaque black to transparent,
            covering the channel-name / video-title info bar that YouTube
            injects at the top of every embed.
          • Right control cap — solid dark block over "More videos" and
            the fullscreen-escape button at the right edge of the control
            bar. pointer-events unset so taps are absorbed (prevents
            accidental navigation to YouTube recommendations).
          • Left control cap  — dark block over YouTube's "Share" /
            external-link button at the left of the control bar.

          Not applied in radio mode (player is 1×1 hidden) or while
          loading (iframe hasn't rendered yet so the caps would flash
          before the poster disappears). */}
      {!isRadioMode && !loading && !reconnecting && (
        <>
          ...
          {!isBroadcastLive && React.createElement("div", {
            key: "tt-rhs",
            "aria-hidden": "true",
            style: {
              position: "absolute",
...
const styles = StyleSheet.create({
  // position:"relative" is explicit so the absolutely-positioned chrome-blocker
  // divs and loading overlays always use THIS div as their containing block,
  // regardless of what react-native-web may or may not set as a default.
  container: { flex: 1, backgroundColor: "#000", overflow: "hidden", position: "relative" },
  // position:"relative" creates a stacking context for the YT iframe that
  // onReady styles as position:absolute — keeps it inside the flex cell.
  playerInner: { flex: 1, position: "relative", backgroundColor: "#000" },
