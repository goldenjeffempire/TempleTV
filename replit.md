...
**Broadcast-ready admission is instant and self-healing.** `isPlayableForBroadcast()` (`modules/broadcast/auto-enqueue.service.ts`) admits a raw MP4 the moment its blob is confirmed committed (`s3MirroredAt` non-null) — FastStart (moov relocation) is never invoked in the automated upload flow and is not an admission gate. `enqueueIfMissing()` now self-heals inline: if the only blocking reason is a missing `s3MirroredAt` stamp (e.g. the post-assembly UPDATE silently failed), it runs a scoped, synchronous blob-existence check (`repairMissingS3MirroredAt(videoId)`) and retries admission in the same call — every caller (upload finalize, reconciler, library scan, manual force-enqueue) gets this for free, so a video never has to wait for the next 60 s reconciler tick or an operator click. The `POST /broadcast-v2/force-enqueue/:videoId` "not yet broadcast-ready" message no longer references FastStart (it never ran) and now only fires when the blob is genuinely still assembling or missing.

## Production deployment topology (Replit VM)

The published Replit deployment (`vm` target, `.replit` `[deployment]`) runs **two independent OS processes** inside the one VM, managed by `scripts/prod-supervisor.mjs` (dependency-free Node, no build step):

- **Broadcast daemon** (`RUN_MODE=broadcast`, loopback-only on `127.0.0.1:9000`) — owns the broadcast engine exclusively: orchestrator, queue, yt-shuffle, checkpointing. Never touched by API-level crashes or restarts.
- **API server** (`RUN_MODE=all`, public port `8080`, `BROADCAST_DAEMON_URL=http://127.0.0.1:9000`) — serves HTTP/admin/auth/uploads and proxies all broadcast SSE/WS/REST traffic to the daemon (`daemon-proxy.ts`, resilient retry/backoff/buffering).

The supervisor starts the daemon first (waits for `/healthz`), then the API; if either child crashes it restarts **only that child** with exponential backoff — the other keeps running. On SIGTERM (redeploy/stop) it drains the API first, then the daemon (which flushes its playback checkpoint to `broadcast_runtime_state` before exiting). A full container replacement (an actual new deploy) is the only event that restarts both together, and the daemon rehydrates the exact playback position from the DB checkpoint in well under a second — verified via `[broadcast-v2] restored cycle anchor from runtime state after restart` / `[yt-shuffle] RESUMED after restart` log lines.

**Known platform limits:** Replit VM deployments run a single publishable process tree — there is no supported way to have the daemon survive a full redeploy untouched (that would need a second, independently-published always-on service, which Replit does not offer for this project shape). The supervisor pattern is the closest available approximation: it isolates the daemon from everything *except* an actual redeploy, and the checkpoint/hydrate system makes even that case sub-second and imperceptible to viewers. `render.yaml` (legacy/alternate deploy target) still runs a single `RUN_MODE=all` process — Render's free tier has no worker-service tier for a true second process, so it doesn't have the same daemon isolation; adopting the same supervisor script there would work if the user ever needs it.
