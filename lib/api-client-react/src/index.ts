...
// row (sentCount + audienceSize). Adapter derives the page-friendly shape.
type SendPushResult = { sent: number; failed: number };
type RawSendPushResponse = SentNotification & { audienceSize: number };
export function useSendPushNotification(opts?: MutationOpts<SendPushResult, SendPushVars>) {
  ...
  return useMutation<SendPushResult, ApiError, SendPushVars>({
    mutationFn: async ({ data }: SendPushVars) => {
      ...
      return {
        sent: raw.sentCount,
        failed: Math.max(0, raw.audienceSize - raw.sentCount),
      };
    },
    onError: opts?.mutation?.onError,
    onSettled: opts?.mutation?.onSettled,
    ...
    mergeQueryOpts<ListAdminVideosResponse>(
      {
        ...
        queryFn: async () => {
          const res = await request<ListEnvelope<RawAdminVideo>>("GET", "/media", {
            query: {
          ...
          const totalPages = Math.max(1, Math.ceil(res.total / limit));
          // Optional client-side source filter — the API doesn't expose it.
          let videos = res.items.map(toAdminVideo);
          if (params?.source && params.source !== "all") {
            videos = videos.filter((v) => v.videoSource === params.source);
          }
          return { videos, total: res.total, totalPages, page, limit };
        },
      },
      opts,
    ),
